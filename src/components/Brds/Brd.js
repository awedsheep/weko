...
function Brd({ name, data, cat }) {
	...
	var navigation = <div className="paging">
	...
	var navigationWithItem = <div className="paging">
		<Link to={{pathname:(basePage * 5) !== 0 ? "/" + cat + "/" + (basePage * 5) + "/"+ itemAbsoluteNum: "/" + cat + "/1/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className="paging_bt">
			<Icon name="angle double left" />
		</Link>
		<Link to={{pathname:((page - 1) >= 1) ? "/" + cat + "/" + (page - 1)+ "/"+ itemAbsoluteNum : "/" + cat + "/" + (page)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className="paging_bt">
			<Icon name="angle left" />
		</Link>
		{(basePage * 5 + 1) <= upUntilPage ? <Link to={{pathname:"/" + cat + "/" + (basePage * 5 + 1)+ "/"+ itemAbsoluteNum , item: postings[itemAbsoluteNum]}} className={page % 5 === 1 ? "paging_number on" : "paging_number"}>
			{basePage * 5 + 1}
		</Link> : <></>}
		{(basePage * 5 + 2) <= upUntilPage ? <Link to={{pathname:"/" + cat + "/" + (basePage * 5 + 2)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className={page % 5 === 2 ? "paging_number on" : "paging_number"}>
			{basePage * 5 + 2}
		</Link> : <></>}
		{(basePage * 5 + 3) <= upUntilPage ? <Link to={{pathname:"/" + cat + "/" + (basePage * 5 + 3)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className={page % 5 === 3 ? "paging_number on" : "paging_number"}>
			{basePage * 5 + 3}
		</Link> : <></>}
		{(basePage * 5 + 4) <= upUntilPage ? <Link to={{pathname:"/" + cat + "/" + (basePage * 5 + 4)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className={page % 5 === 4 ? "paging_number on" : "paging_number"}>
			{basePage * 5 + 4}
		</Link> : <></>}
		{(basePage * 5 + 5) <= upUntilPage ? <Link to={{pathname:"/" + cat + "/" + (basePage * 5 + 5)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className={page % 5 === 0 ? "paging_number on" : "paging_number"}>
			{basePage * 5 + 5}
		</Link> : <></>}
		<Link to={{pathname:((page + 1) <= upUntilPage) || postings.length % 100 === 0 ? "/" + cat + "/" + (page + 1)+ "/"+ itemAbsoluteNum : "/" + cat + "/" + (page)+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className="paging_bt">
			<Icon name="angle right" />
		</Link>
		<Link to={{pathname:(basePage * 5 + 5) <= upUntilPage ? "/" + cat + "/" + (basePage * 5 + 6) + "/"+ itemAbsoluteNum: "/" + cat + "/" + upUntilPage+ "/"+ itemAbsoluteNum, item: postings[itemAbsoluteNum]}} className="paging_bt">
			<Icon name="angle double right" />
		</Link>
		<div className="writeButton">
			<Link
		...
		<div className="brd">
			...
			<div className="brd_body">
				{postings.slice((page - 1) * 20, (page) * 20).map((item, i) => {
					return (
						<BrdItem  name={name} key={i} cat={cat} page={page} num={(page-1)*20+i} />
					);
				})}
			</div>
